...
async function run() {
  try {
    ...
    await client.connect();
     const menuCollection = client.db("pakwaanDb").collection("menu");
     const userCollection = client.db("pakwaanDb").collection("users");
    const reviewCollection = client.db("pakwaanDb").collection("reviews");
    const cartsCollection = client.db("pakwaanDb").collection("carts");
...
app.patch('/user/:email',async(req,res)=>{
    ...
    const result=await userCollection.updateOne({email},{$set:userData },{
        upsert:true
    })
    res.send(result)
})
...
app.delete('/users/:id', verifyToken, async(req,res)=>{
    ...
    //     const searchResults = await menuCollection.find(query);
    ...
    app.post('/menu/search',async(req,res)=>{
     const searchValue = req.body.searchValue;
      console.log(searchValue);
      const query = { name: 'HADDOCK' };
      const searchResults=await menuCollection.findOne(query)
      if (searchResults) {
